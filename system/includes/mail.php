...
function sendMail($gonderenAdi,$konu,$mesaj,$aliciAdresi = null, $use_theme = true)
{
	if(trim(get_option("mailUser")) != "")
	{
		...
		if($use_theme)
		{
			...
			$mailer = str_replace('<%siteTitle%>', get_option("siteTitle"), $mailer);
			$mailer = str_replace('<%subject%>', $konu, $mailer);
			$mailer = str_replace('<%message%>', $mesaj, $mailer);
			$mesaj = $mailer;
		}
		
		$konu = get_option("siteTitle") . " - " . $konu;
		
		if(!is_array($aliciAdresi))
			$aliciAdresi = ($aliciAdresi == null) ? get_option("getMailAddress") : $aliciAdresi;
		
		if(trim(get_option("isSmtpMail")) == "")
		{
			$kimden = get_option("mailUser");
		...
		else
		{
			...
			$MAIL->Host = get_option("mailHost"); // SMTP server
			$MAIL->Username = get_option("mailUser");
			$MAIL->Password = get_option("mailPassword");
			$MAIL->Port = get_option("mailPort");
			
			$MAIL->IsHTML(true); // send as HTML
			$MAIL->CharSet = "UTF-8";
			
			$MAIL->From = get_option("mailUser");
